...
byte ht1632c::putchar(uint8_t x, uint8_t y, char c, byte color, byte attr)
{
  word dots, msb;
...
// void ht1632c::putInt(int x, int y, int num, byte color)
// {
  ...
  // width++;
      ...
      // {
			// showcolor = (color & RANDOMCOLOR) ? random(1,4) : color;
			// showcolor = ((color & BLINK) && (x & 2)) ? BLACK : (showcolor & ~BLINK);
			// putchar(x + width * i,  y, charBuf[i], showcolor);
      // }
      // sendframe();
// }

void ht1632c::putInt(uint8_t y, int num, byte color, boolean left)
{
  String stringOne = String(num);
  char charBuf[6];
  stringOne.toCharArray(charBuf,6);
  byte showcolor;
  uint8_t len = strlen(charBuf);
  if(len>5) len = 5;
  ...
  int8_t diff = lastLength - len;
  if(diff>0){
    for(uint8_t i = 0;i<diff;i++){
      x = k + width * (lastLength - (i + 1));
      putchar(x, y, ' ', BLACK);
    }
  }
  ...
  for (uint8_t i = 0; i<len; i++){
    ...
    showcolor = (color & RANDOMCOLOR) ? random(1,4) : color;
    showcolor = ((color & BLINK) && (x & 2)) ? BLACK : (showcolor & ~BLINK);
    putchar(x,  y, charBuf[i], showcolor);
  }
  sendframe();
  lastLength = len;
}
